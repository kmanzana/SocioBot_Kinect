...
vector<double> calculateAngles(const NUI_SKELETON_DATA& skel);
...
void CSkeletonBasics::ProcessSkeleton()
{
	...
	calculateAngles(skeletonFrame.SkeletonData[0]);
...
double calculateAlpha(const Vector4* childPoints, int elbow, int shoulder, int wrist);
double calculateBeta(const Vector4* childPoints, int elbow, int shoulder);
double calculateGamma(const Vector4* childPoints, int elbow, int shoulder, int wrist);

///<summary>
///Calculates Alpha, Beta, and Gamma for a given skeleton
///</summary>
///<param name="skeleton"> which skeleton to calculate things for </param>

vector<double> calculateAngles(const NUI_SKELETON_DATA& skel){
	const Vector4* childPoints = skel.SkeletonPositions;

	double rightAlpha = calculateAlpha(childPoints, NUI_SKELETON_POSITION_ELBOW_RIGHT,
	                                                NUI_SKELETON_POSITION_SHOULDER_RIGHT,
		                                            NUI_SKELETON_POSITION_WRIST_RIGHT);
	double leftAlpha = calculateAlpha(childPoints,  NUI_SKELETON_POSITION_ELBOW_LEFT,
		                                            NUI_SKELETON_POSITION_SHOULDER_LEFT,
		                                            NUI_SKELETON_POSITION_WRIST_LEFT);
	
	double rightBeta = calculateBeta(childPoints, NUI_SKELETON_POSITION_ELBOW_RIGHT, NUI_SKELETON_POSITION_SHOULDER_RIGHT);
	double leftBeta  = calculateBeta(childPoints, NUI_SKELETON_POSITION_ELBOW_LEFT , NUI_SKELETON_POSITION_SHOULDER_LEFT );

	double rightGamma = calculateGamma(childPoints, NUI_SKELETON_POSITION_ELBOW_RIGHT,
					                                NUI_SKELETON_POSITION_SHOULDER_RIGHT,
													NUI_SKELETON_POSITION_WRIST_RIGHT);
...
double calculateAlpha(const Vector4* childPoints, int elbow, int shoulder, int wrist){
	double top = (childPoints[shoulder].z - childPoints[elbow].z)
		       * (childPoints[wrist].x    - childPoints[elbow].x)
		       - (childPoints[shoulder].x - childPoints[elbow].x)
		       * (childPoints[wrist].z    - childPoints[elbow].z);
	double bottom = pow((childPoints[shoulder].z - childPoints[elbow].z)
		              * (childPoints[wrist].x    - childPoints[elbow].x)
		              - (childPoints[shoulder].x - childPoints[elbow].x)
		              * (childPoints[wrist].z    - childPoints[elbow].z), 2)
		          + pow((childPoints[shoulder].y - childPoints[elbow].y)
		              * (childPoints[wrist].z    - childPoints[elbow].z)
		              - (childPoints[shoulder].z - childPoints[elbow].z)
		              * (childPoints[wrist].y    - childPoints[elbow].y), 2)
		          + pow((childPoints[shoulder].x - childPoints[elbow].x)
	                  * (childPoints[wrist].y    - childPoints[elbow].y)
		              - (childPoints[shoulder].y - childPoints[elbow].y)
		              * (childPoints[wrist].x    - childPoints[elbow].x), 2);
	return acos(top / sqrt(bottom));
}

double calculateBeta(const Vector4* childPoints, int elbow, int shoulder){
	double top = childPoints[elbow].x - childPoints[shoulder].x;
	double bottom = pow(childPoints[elbow].x - childPoints[shoulder].x, 2)
		          + pow(childPoints[elbow].y - childPoints[shoulder].y, 2)
				  + pow(childPoints[elbow].z - childPoints[shoulder].z, 2);
	
	return acos(top / sqrt(bottom));
}

double calculateGamma(const Vector4* childPoints, int elbow, int shoulder, int wrist){
	double top = (childPoints[shoulder].x - childPoints[elbow].x)
		       * (childPoints[wrist].x    - childPoints[elbow].x)
		       + (childPoints[shoulder].y - childPoints[elbow].y)
		       * (childPoints[wrist].y    - childPoints[elbow].y)
		       + (childPoints[shoulder].z - childPoints[elbow].z)
	...
	double bottomLeft = pow(childPoints[shoulder].x - childPoints[elbow].x, 2)
		              + pow(childPoints[shoulder].y - childPoints[elbow].y, 2)
					  + pow(childPoints[shoulder].z - childPoints[elbow].z, 2);
	double bottomRight = pow(childPoints[wrist].x - childPoints[elbow].x, 2)
		               + pow(childPoints[wrist].y - childPoints[elbow].y, 2)
					   + pow(childPoints[wrist].z - childPoints[elbow].z, 2);
	return acos(top / (sqrt(bottomLeft) * sqrt(bottomRight)));
}
